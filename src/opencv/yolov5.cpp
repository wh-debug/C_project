...
/***************************以下代码不同***************************/
    int fcount = 0;
    for (int f = 0; f < (int)file_names.size(); f++) {　//这句被注释掉
        fcount++;
        if (fcount < BATCH_SIZE && f + 1 != (int)file_names.size()) continue;　//这句不同被注释掉
        for (int b = 0; b < fcount; b++) {
            cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]); //这句不同
